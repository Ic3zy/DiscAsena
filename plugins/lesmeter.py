...
if not auth==MAİN_AUTH:
    MAİN_AUTH=auth
class LezMeet(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
    @commands.command()
    async def lez(self, ctx, member: commands.MemberConverter = None):
        usr=str(ctx.author)
        if WORKTYPE=="private":
            if usr==AUTHOR or usr==MAİN_AUTH:         
                ...
                """if member:
                    ...
                    if ctx.message.reference:
                        ...
                        await ctx.send(f'{user.mention} Hesaplandı:{sor}')"""
                if member:
                    await ctx.send(f'{member.mention} Kişinin lez yüzdesi hesaplanıyor...')
                    time.sleep(1)
                    ...
                    if ctx.message.reference:
                        replied_message = await ctx.fetch_message(ctx.message.reference.message_id)
                        user = replied_message.author
                        await ctx.send(f'{user.mention} Kişinin lez yüzdesi hesaplanıyor...')
                        time.sleep(1)
                        await ctx.send(f'%{sor}')
                    else:
                        await ctx.send(f'{ctx.author.mention} Kişinin lez yüzdesi hesaplanıyor...')
                        time.sleep(1)
                        await ctx.send(f'%{sor}')
